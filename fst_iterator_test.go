...
func TestIteratorExists(t *testing.T) {
	var buf bytes.Buffer
	b, err := New(&buf, nil)
	if err != nil {
		t.Fatalf("error creating builder: %v", err)
	...
	exists, err = it.Exists([]byte("cat"))
	if exists {
		t.Fatalf("`cat` shouldn't exist in the fst")
	}
	for err != nil && err != ErrIteratorDone {
		t.Fatalf("find failed, err: %v", err)
	}

	exists, err = it.Exists([]byte("tye"))
	if !exists {
		t.Fatalf("`tye` should exist in the fst")
	}
	for err != nil && err != ErrIteratorDone {
		t.Fatalf("find failed, err: %v", err)
	}

	exists, err = it.Exists([]byte("tyes"))
	if exists {
		t.Fatalf("`tyes` shouldn't exist in the fst")
	}
	for err != nil && err != ErrIteratorDone {
		t.Fatalf("find failed, err: %v", err)
	}

	exists, err = it.Exists([]byte("monday"))
	if exists {
		t.Fatalf("`tyes` shouldn't exist in the fst")
	}
	for err != nil && err != ErrIteratorDone {
	...
	exists, err = it.Exists([]byte("mon"))
	if !exists {
		t.Fatalf("`mon` should exist in the fst")
	}
	for err != nil && err != ErrIteratorDone {
		t.Fatalf("find failed, err: %v", err)
	}
